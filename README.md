...
- jq（AWS CLI コマンドで `cdk.json` から設定値を読み取るために使用）
...
以降の AWS CLI コマンドでは、リージョンや送信元アドレスを `cdk.json` から `jq` で直接読み取っています（例: `$(jq -r '.context.sesConfig.region' cdk.json)`）。シェル変数を事前に設定する必要はなく、**各コマンドをそのままコピー＆ペーストで実行できます。**
...
aws ses verify-email-identity \
  --email-address $(jq -r '.context.sesConfig.senderEmail' cdk.json) \
...
aws ses get-identity-verification-attributes \
  --identities $(jq -r '.context.sesConfig.senderEmail' cdk.json) {送信先メールアドレス} \
  --region $(jq -r '.context.sesConfig.region' cdk.json)
```
...
```bash
curl -X POST "https://{API ID}.execute-api.$(jq -r '.context.sesConfig.region' cdk.json).amazonaws.com/$(jq -r '.context.sesConfig.stageName' cdk.json)/send" \
...
```bash
curl -X POST "https://{API ID}.execute-api.$(jq -r '.context.sesConfig.region' cdk.json).amazonaws.com/$(jq -r '.context.sesConfig.stageName' cdk.json)/send-template" \
  -H "Content-Type: application/json" \
  -H "x-api-key: {APIキーのvalue}" \
  -d '{
    "to": ["{送信先メールアドレス}"],
...
aws ses delete-identity \
  --identity $(jq -r '.context.sesConfig.senderEmail' cdk.json) \
  --region $(jq -r '.context.sesConfig.region' cdk.json)
